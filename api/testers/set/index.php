<?php
/**
 * API to create a set a testers names to a session
 * api/testers/set/index.php?names[]=[arrayOfUserNames]&sessionid=[sessionId]
 */
...
if (isset($_REQUEST['names']) && isset($_REQUEST['sessionid']) && $_REQUEST['names'] != null) {
    ...
    if ($so->getSessionExist()) {
        ...
        if ($sHelper->isUserAllowedToEditSession($so)) {
            $sqlDelete = "DELETE FROM mission_testers WHERE versionid='" . $so->getVersionid() . "'";
            $result = dbHelper::sw_mysqli_execute($con, $sqlDelete, __FILE__, __LINE__);
            $logger->debug("Deleted all testers that was co-tester in session $sessionid", __FILE__, __LINE__);

            foreach ($testers as $name) {
                $sql = "INSERT INTO mission_testers (versionid, tester) VALUES ('" . $so->getVersionid() . "', '$name');";
                $result = dbHelper::sw_mysqli_execute($con, $sql, __FILE__, __LINE__);
                $logger->debug("Added user name $name as a co-tester in session $sessionid", __FILE__, __LINE__);
            }
        ...
        $logger->debug("Tried to change co-testers but sessionid $sessionid does not exist", __FILE__, __LINE__);
    ...
    $logger->debug("Tried to change co-testers but one of the parameters is bad", __FILE__, __LINE__);
