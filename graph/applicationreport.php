...
if (isset($_REQUEST['sprint'])) {
    ...
    echo "<div>Sprint: ";
    HtmlFunctions::echoSprintSelect("", true);
    echo "</div>";
...
function getTeamOrApplicationStatistics($allSessions)
{
    ...
    $allApplicationsBasedOnAreaName = QueryHelper::getApplicationsFromAreaNames();

    //To get all "applications" into an array.
    foreach ($allSessions as $sessionId => $aSession) {
        $areas = $aSession['areas'];
        foreach ($areas as $area) {
            $appName = QueryHelper::getApplicationsFromAreaNames($area);
