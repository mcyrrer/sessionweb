...
function printTeamStatistics($sprint)
{
    ...
    $resultTeam = $dbm->executeQuery($con,$sql);
    $totalCount = 0;
...
function printAreaStatistics($sprint)
{
    ...
    while ($row = mysqli_fetch_array($resultTeam)) {
        ...
        $resultTeamSessions = $dbm->executeQuery($con,$sql);
        $nbrOfSessions = mysql_num_rows($resultTeamSessions);
        while ($rowSessions = mysqli_fetch_array($resultTeamSessions)) {
...
function getAreasConnectedToSession($rowSessions, $areasArray, $totalTimeInSessions)
{
    ...
    $resultTeamAreas = $dbm->executeQuery($con,$sqlAreas);
    while ($rowTeamAreas = mysqli_fetch_array($resultTeamAreas)) {
